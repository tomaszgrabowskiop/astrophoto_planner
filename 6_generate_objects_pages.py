...
from shared import (PATHS, 
                    ...
                    MONTH_NAMES, TYPE_NAMES, CATALOG_PRIORITY, CAT_ORDER,
                    H_START, H_END, H_RANGE, H_Y_RANGE, H_Y_START,
                    N_SAMPLES, CROSSING_SAMPLES,
                    get_engine_raw_hash, get_engine_final_hash, 
                    load_observing_data, extract_famous_labels, build_badge, smart_truncate)

# --- KONFIGURACJA ---
vis_data = load_vis_data(PATHS.vis_data)   # vis_data.json
user = UserConfig.from_vis_data(vis_data)  # obiekt konfiguracyjny

YEAR = user.year
LAT = user.location.lat
LON = user.location.lon
LOCATION = EarthLocation(lat=LAT * u.deg, lon=LON * u.deg)
MIN_ALTITUDE = user.min_altitude

camera_cfg = user.camera
...
def compute_diagonal_fov_arcmin(camera_cfg: CameraConfig):
    _, _, d_deg = camera_cfg.calculate_fov()  # width, height, diagonal
    fov_arcmin = d_deg * 60.0
    return fov_arcmin, d_deg
...
def format_indeksy(extra_info: str, max_items: int = 24, max_chars: int = 65, max_lines: int = 3) -> str:
    """Usuwa słynne (M/C/H), sortuje resztę wg CATALOG_PRIORITY."""
    if not extra_info:
        return 'brak'
    
    # 1. Wyciągnij i USUŃ słynne
    famous = set(extract_famous_labels(extra_info))

    # 2. Filtruj items: pomiń słynne
    items = [item.strip() for item in extra_info.split(',') if item.strip() not in famous]
    
    # 3. Sortuj wg CATALOG_PRIORITY (malejąco prio, rosnąco alfabetycznie)
    def get_priority(item: str) -> tuple:
        ...
        prio = CATALOG_PRIORITY.get(prefix, 0)
    ...
    if len(short_items) <= items_per_line:
        ...
        if len(line) > max_chars:
            line = smart_truncate(line, (max_chars-3))
        return line
    else:
        lines = []
        # Podziel na chunki
...
def draw_object_page(pdf, oid, month, nm_day, row, all_data, camera_cfg, page_num, tz, best_year_date, best_year_m_hours,):
    ...
    short_name = smart_truncate(raw_name,(max_len - 3)) if len(raw_name) > max_len else raw_name
    base_id = row.get("id", "")
    badge_str = build_badge(row)
    name_display = f"{base_id}{badge_str}"
    ...
    t_start = tz.localize(datetime(YEAR, month, nm_day, int(H_START), 0))
    h_rel = np.linspace(0, H_RANGE, 150)
    ...
    ax_nm.plot(h_rel, o_alt, color="#4A148C", lw=2)
    ax_nm.plot(h_rel, m_alt, color="#003333", lw=1, ls="--", alpha=0.7)
    ax_nm.axhline(MIN_ALTITUDE, color="#880E4F", ls=":", lw=1)

    ax_nm.set_ylim(0, 90)
    ax_nm.set_xlim(0, H_RANGE)
    ax_nm.set_xticks(np.arange(0, H_RANGE, 1))
    ...
    # 3. Kadr FOV
    fov_arcmin, fov_deg = compute_diagonal_fov_arcmin(camera_cfg)
    img_path = PATHS.starplots / f"{oid}.png"
    ...
    ax_vis.set_ylim(H_Y_START, H_Y_RANGE)
    y_ticks = np.arange(H_Y_START, H_Y_RANGE + 0.1, 1)
    ...
    ax_vis.grid(which="major", axis="x", color="white", linewidth=0.7, alpha=0.9, zorder=3)
    ax_vis.grid(which="minor", axis="x", color="white", linewidth=1, alpha=1, zorder=3)

    ax_vis.tick_params(axis="x", which="minor", length=8, pad=5, labelsize=8, color="gray")
    ax_vis.tick_params(axis="x", which="major", length=4, pad=2, labelsize=5, color="gray")

    ax_vis.set_xlim(days_pd[0], days_pd[-1])
    ax_vis.yaxis.set_minor_locator(ticker.MultipleLocator(0.25))
    ax_vis.grid(which="major", axis="y", color="white", linewidth=0.7, alpha=1, zorder=3)
    ax_vis.grid(which="minor", axis="y", color="white", linewidth=0.2, alpha=0.9, zorder=3)
    ...
    ax_hrs.set_ylim(0, int(H_START - 1))
...
def draw_context_page(pdf, oid, page_num):
    ...
    img_path = PATHS.starplots / f"context_{oid}.png"
    try:
        img = plt.imread(img_path)
...
def main():
    ...
    all_data = load_observing_data(PATHS.observing_final)
    ...
    #df = df.iloc[21:22] # Włącz zakres obiektów do opracowania, indeksy są od 0
    ...
    with PdfPages(output_name) as pdf:
        ...
        for _, row in pbar:
            ...
            draw_object_page(pdf, oid, month, nm_day,  row, all_data, camera_cfg, page_num,  tz,  best_year_date, best_year_m_hours,)
            page_num += 1
