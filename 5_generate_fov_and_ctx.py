...
def _worker_render_fov(args):
    ...
    """
    obj, camera_config, out_dir = args
    
    try:
        camera = create_camera_object(camera_config)
...
def create_camera_object(cfg: CameraConfig) -> Camera:
    """
    Tworzy obiekt Camera na podstawie słownika.
    Ta funkcja będzie wywoływana WEWNĄTRZ procesu workera.
    """
    return Camera(
        lens_focal_length=cfg.lens_focal_length,
        sensor_pitch=cfg.sensor_pitch,
        sensor_rows=cfg.sensor_rows,
...
# Field of view
#
def generate_fov_pngs(objs: List[Dict[str, Any]], camera_conf: CameraConfig) -> List[Path | None]:
    PATHS.starplots.mkdir(exist_ok=True, parents=True)
    ...
    for obj in objs:
        ...
        expected_path = PATHS.starplots / f"{name}.png"
        ...
        if expected_path.exists():
            ...
            tasks.append((obj, camera_conf, PATHS.starplots))
    ...
    with ProcessPoolExecutor(max_workers=mp.cpu_count()) as executor:
        ...
        with tqdm(total=len(tasks), desc="       Engine FOV", unit="obj", ncols=119, colour="green") as pbar:
...
def generate_context_pngs(objs: List[Dict[str, Any]]) -> List[Path | None]:
    ...
    for obj in objs:
        name = str(obj.get("id") or obj.get("name"))
        expected_path = PATHS.starplots / f"context_{name}.png"

        if expected_path.exists():
            all_results[name] = expected_path
            skipped_count += 1
        else:
            tasks.append((obj, PATHS.starplots))
    ...
    with ProcessPoolExecutor(max_workers=mp.cpu_count()) as executor:
        ...
        with tqdm(total=len(tasks), desc="       Engine CTX", unit="obj", ncols=119, colour="green") as pbar:
            for future in as_completed(future_to_objid):
                obj_id = future_to_objid[future]
                try:
                    out_path = future.result()
...
def main() -> None:
    print_green("=" * 119)
    print_green("GENEROWANIE GRAFIK: FOV & MAPY KONTEKSTOWE")
    print_green("=" * 119)
    print_step("Pobieram katalogi dla StarPlot.")
    preload_open_ngc()
    # 1. Ładujemy dane przy użyciu shared
    vis_data_json = load_vis_data(PATHS.vis_data)
    # 2. Tworzymy obiekt konfiguracji (automatycznie parsuje kamerę itp.)
    user_config = UserConfig.from_vis_data(vis_data_json)
    cam = user_config.camera
    w_deg, h_deg, d_deg = cam.calculate_fov()
    print("[INFO] Konfiguracja kamery:\n"
        f"       ogniskowa:           {cam.lens_focal_length} mm,\n"
        f"       rozmiar sensora:     {cam.sensor_width} x {cam.sensor_height} mm,\n"
        f"       rozdzielczość:       {cam.sensor_cols} x {cam.sensor_rows} px,\n"
        f"       wielkość piksela:    {cam.sensor_pitch} µm,\n"
        f"       pole widzenia:       {w_deg:.2f}° x {h_deg:.2f}° (przekątna {d_deg:.2f}°)."
    )

    # 3. Wyciągamy obiekty
    raw_objects = vis_data_json.get("objects", [])
    selected_objects = [o for o in raw_objects if o.get("selected")]
    if not selected_objects:
        print_step("Brak obiektów z polem 'selected'.")
        return
    print_step(f"Wybrano {len(selected_objects)} obiektów (wszystkie z 'selected' w vis_data.json).")
    # Upewnij się, że katalog istnieje
    PATHS.starplots.mkdir(exist_ok=True)
    # 4. Uruchamiamy generowanie FOV
    # Przekazujemy user_config.camera (dataclass)
    generate_fov_pngs(selected_objects, user_config.camera)
    # 5. Uruchamiamy generowanie Context
    generate_context_pngs(selected_objects)
    print_step(f"Gotowe. Wygenerowane pliki znajdują się w folderze {Path(PATHS.starplots).resolve().as_posix()}.")
